...
if (opcaoCompra == 1){
  ...
  print("Para pintar ${espacoPintar.toStringAsFixed(2)}m², serão necessárias $galoesAComprar galões de tinta, o que resultam em R\$${valorTotal.toStringAsFixed(2)}!!!");

}else if (opcaoCompra == 3) {
  
  int latasAComprar = (litrosComFolga / litrosLata).floor(); // Quantidade de latas
  double restanteLitros = litrosComFolga - (latasAComprar * litrosLata); // Quantos litros faltam

  // Calcular o número de galões necessários para o restante
  int galoesAComprar = (restanteLitros / litrosGalao).ceil();

  double valorTotal = (galoesAComprar * valorGalao) + (latasAComprar * valorLata);

  print("Para pintar ${espacoPintar.toStringAsFixed(2)}m², serão necessárias $latasAComprar latas de tinta e $galoesAComprar galões de tinta, o que resulta em R\$${valorTotal.toStringAsFixed(2)}!!!");
  }
